...
describe("GeminiTicketClassifierProvider", () => {
  it("sends a structured output request and parses the classification", async () => {
    let capturedRequest;
    const provider = new GeminiTicketClassifierProvider({
      apiKey: "test-key",
      model: "test-model",
      fetchImpl: async (url, request) => {
        ...
        return {
          ok: true,
          json: async () => geminiPayload(validClassification)
        };
      }
    });
  ...
  it("fails without an API key", () => {
    assert.throws(
      () =>
        new GeminiTicketClassifierProvider({
          apiKey: ""
        }),
      /GEMINI_API_KEY/
    );
  ...
  it("fails when Gemini returns a non-success response", async () => {
    const provider = new GeminiTicketClassifierProvider({
      apiKey: "test-key",
      fetchImpl: async () => ({
        ok: false,
  ...
  it("sends a structured copilot draft request", async () => {
    ...
    const provider = new GeminiTicketClassifierProvider({
      ...
      fetchImpl: async (url, request) => {
        ...
        return {
          ok: true,
          json: async () => geminiPayload(validCopilotDraft)
        };
      }
    });
...
describe("parseGeminiStructuredOutput", () => {
  it("parses response text from candidate parts", () => {
    const result = parseGeminiStructuredOutput(geminiPayload(validClassification));
